function makeLineChart(dataset, xName, yObjs, axisLables) {
  ...
  chartObj.update_svg_size = function () {
    ...
    // Handle translation with NaN gracefully
    if (!isNaN(chartObj.height)) {
...
for (var y in yObjs) {
  ...
  // Log the path attribute data for each line
  console.log("Path attribute data for", y, ":", yObjs[y].line);
}
    ...
    function mousemove() {
      ...
      // Check if chartObj.height is a valid number
      if (!isNaN(chartObj.height)) {
...
function initializeChart() {
  ...
  var chart = makeLineChart(data, 'year', {
      'Palestine': { column: 'Palestine' },
      'Israel': { column: 'Israel' }
  }, { xAxis: 'Years', yAxis: 'Amount' });

  // Bind the chart to the specified container
